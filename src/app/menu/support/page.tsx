  ...
  return (
    <PageContainer>
      ...
      <main className="flex-1 px-outer py-[22px]">
        ...
        <p className="mt-[28px] pb-[6px] font-serif text-[22px] font-bold text-text">
          {t("support.faq")}
        </p>
        {FAQS.map((f, i) => {
          ...
          return (
            <div key={f.qKey} className={i !== FAQS.length - 1 ? "border-b border-divider" : ""}>
              ...
              >
                <span className="font-sans text-[15.5px] font-medium text-text">{t(f.qKey)}</span>
                <ChevronDownIcon className={`h-[15px] w-[15px] shrink-0 text-muted transition-transform duration-200 ${open ? "rotate-180" : ""}`} />
              </button>
              {open && <p className="anim-fade-through pb-[13px] font-sans text-[15px] leading-[1.5] text-muted">{t(f.aKey)}</p>}
            </div>
          );
        })}

        <p className="mt-[28px] pb-[8px] font-serif text-[22px] font-bold text-text">
          {t("support.contactForm")}
        </p>
        {sent ? (
          <p className="rounded-card bg-soft-surface px-[16px] py-[14px] font-sans text-[15px] text-navy">
            {t("support.thankYou")}
          </p>
        ) : (
          <form
          ...
          >
            <textarea
              ...
              className="w-full rounded-sm border border-divider bg-surface px-[14px] py-[12px] font-sans text-[15.5px] text-text outline-none focus:border-navy"
            />
            <button
              type="submit"
              className="press mt-[10px] w-full rounded-pill bg-navy py-[13px] font-sans text-[15.5px] font-semibold text-white"
            >
